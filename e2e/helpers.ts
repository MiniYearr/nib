...
/** Launches the built app (electron-vite build must have run) against an isolated data dir. */
export async function launchNib(userData?: string): Promise<LaunchedApp> {
  const dataDir = userData ?? mkdtempSync(join(tmpdir(), 'nib-e2e-'));
  const app = await electron.launch({
    executablePath: require('electron') as unknown as string,
    args: [desktopDir],
    env: { ...process.env, NIB_USER_DATA: dataDir },
  });
